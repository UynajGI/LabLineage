  ...
  try {
    ...
    await withServer(async (baseUrl) => {
      ...
      const isolationProjectResponse = await fetch(`${baseUrl}/v1/projects`, {
        ...
        body: JSON.stringify({
          name: `Isolation project ${Date.now()}`,
          objective: 'Verify project data isolation across actors.',
          successCriteria: [{ description: 'Each actor only observes their own project.', required: true }]
        })
      });
