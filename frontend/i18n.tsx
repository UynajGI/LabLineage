...
const translations: Record<string, { en: string; zh: string }> = {
  ...
  'The application does not read your project files directly. Collector scans the directory locally and sends signed, path-redacted evidence.': { en: 'The application does not read your project files directly. Collector scans the directory locally and sends signed, path-redacted evidence.', zh: '应用不会直接读取项目文件。Collector 在本机扫描目录，仅发送签名且路径脱敏的证据。' },
  ...
  'Run both commands from the LabLineage Guardian repository on the machine that owns the old project:': { en: 'Run both commands from the LabLineage Guardian repository on the machine that owns the old project:', zh: '在存放旧项目的电脑上，从 LabLineage Guardian 仓库依次运行下面两条命令：' },
