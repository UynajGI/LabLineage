...
export const UploadCenter: React.FC = () => {
  ...
  const startUpload = async () => {
    ...
    try {
      ...
      addLog(error instanceof Error ? error.message : t('Manifest import failed.'), 'error');
    }
  };
  ...
  return (
    <div className="space-y-6 h-full flex flex-col max-w-4xl mx-auto">
      <div>
        ...
        <p className="text-slate-600 mt-1">{t('Import a validated lablineage.manifest.v1 JSON file generated by the Edge Collector.')}</p>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-lg shadow-sm p-8">
        ...
        {uploadState === 'idle' && (
          ...
          >
            ...
            <p className="text-lg font-medium text-slate-700 mb-1">
              {file ? file.name : t('Click or drag a LabLineage manifest.json here')}
            </p>
            <p className="text-sm text-slate-500">
              {file ? `${(file.size / (1024 * 1024)).toFixed(2)} MB` : t('Maximum file size: 5 MB')}
            </p>
          </div>
        )}
        ...
        <div className="mt-8 flex justify-between items-center">
          <div className="flex items-center space-x-2 text-sm text-slate-500">
            <ShieldCheck size={16} />
            <span>{t('The backend validates schema, bundle ID and Ed25519 signature policy before import.')}</span>
          </div>
