...
const getSeverityBadge = (severity: string) => {
  ...
  switch (severity) {
    case 'P0': return <span className={`${baseClasses} bg-red-50 text-red-700 border-red-200`}>P0 Critical</span>;
    case 'P1': return <span className={`${baseClasses} bg-orange-50 text-orange-700 border-orange-200`}>P1 High</span>;
    case 'P2': return <span className={`${baseClasses} bg-yellow-50 text-yellow-700 border-yellow-200`}>P2 Medium</span>;
    case 'P3': return <span className={`${baseClasses} bg-blue-50 text-blue-700 border-blue-200`}>P3 Low</span>;
...
export const FindingsList: React.FC<FindingsListProps> = ({ findings }) => {
  ...
  const resolveFinding = async (finding: Finding) => {
    if (!window.confirm(`Resolve “${finding.title}”? This records your identity and an immutable audit event.`)) return;
    ...
    try {
      ...
      setActionError(error instanceof Error ? error.message : 'Unable to resolve finding');
    } finally {
      setResolvingId('');
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-slate-800">Audit Findings</h2>
      ...
      {currentFindings.length === 0 ? (
        <div className="bg-green-50 border border-green-200 rounded-lg p-8 text-center flex flex-col items-center">
          ...
          <p className="text-green-600">No open findings found in the current snapshot.</p>
        </div>
      ) : (
        <div className="space-y-4">
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <div className="bg-slate-50 rounded p-3 border border-slate-100 mb-3">
                      <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider block mb-1">Affected Entities</span>
                    ...
                    <div>
                      <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider block mb-1">Proposed Action</span>
