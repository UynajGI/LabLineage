        ...
        <div className="space-y-5">
          ...
          {sourceChoice === 'collector' && (
            <section className="rounded-xl border border-slate-200 bg-white p-6">
              <div className="flex items-start gap-3"><ShieldCheck className="text-emerald-600" /><div><h2 className="font-semibold text-slate-900">{t('Connect Local Collector')}</h2><p className="mt-1 text-sm text-slate-600">{t('The application does not read your project files directly. Collector scans the directory locally and sends signed, path-redacted evidence.')}</p></div></div>
              {!pairing ? (
                ...
                <div className="mt-5 space-y-4">
                  ...
                  <div><div className="mb-2 text-sm font-medium text-slate-800">{t('Run both commands from the LabLineage Guardian repository on the machine that owns the old project:')}</div><pre className="overflow-x-auto rounded-lg bg-slate-950 p-4 text-xs text-slate-100"><code>{collectorCommand}</code></pre></div>
