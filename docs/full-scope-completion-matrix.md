...
| 测试与质量 | 单元、契约、集成、安全、Agent 轨迹、性能、E2E、可用性 | 进行中 | 后端 37 项（本机 36 通过、1 项真实 PostgreSQL 门禁跳过）、Collector 17/17、2 项跨 10 页面浏览器 E2E/Axe 和 4 类 Agent 轨迹评测通过；最新 5,000 文件门禁冷 412、热 5,207 files/s，热缓存 100% 命中且目录指纹稳定；本地 Git 适配、对象不可变性、载荷隔离、发布证据防篡改和冷热快照均有测试；待真实 PostgreSQL/外部平台/百万级性能/试点验收 |
| CI/CD 与部署 | 可复现构建、迁移、扫描、制品、环境部署、回滚 | 进行中 | CI 已定义 PostgreSQL service、测试、跨平台 Collector、浏览器 E2E、迁移、critical audit、Trivy、OpenAPI/迁移/幂等/工作流门禁和镜像构建；14 个第三方 Action 引用固定完整 SHA，主分支生成 CycloneDX、Collector 包、SHA-256 与 Sigstore 证据；标准 pre-commit/commit-msg/pre-push/post-commit 已启用；Terraform 创建 Artifact Registry 与仓库限定 GitHub OIDC，CD 更新迁移 Job/Cloud Run、readiness 失败自动回滚；Terraform 1.15.8 本机验证通过，待首次 CI/云执行与回滚演练 |
